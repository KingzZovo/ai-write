...
- `REFERENCE_INGEST_CONCURRENCY` 控制 retry wave 内部 LLM/Qdrant 分支并发；2026-05-17 已按 3→4→5→6 小步上调。Celery worker `--concurrency=4` 暂不调整；为避免中断活跃 retry wave，调参后等待安全维护窗口/当前 wave 完成再重建 worker。
