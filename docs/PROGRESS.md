> **✅ 2026-05-17 14:55 前端创建链路修复**：排查项目目的与创建链路后确认，AI Write 的参考书反编译产物（style/beat cards）应在“创建新书 → 全书大纲 → 分卷/章节 → 正文生成上下文”中闭环使用；但 `NewProjectModal` 只保存 `style_reference.profile_id` 且把 `reference_book_id` 写死为 `null`，`DesktopWorkspace` 第一步也只走普通 `/api/generate/outline`，没有接入已存在的 `/api/outlines/from-reference/start`。已补前端入口：新建项目可选择可用参考书并写入 `settings_json.style_reference.reference_book_id`；工作区生成全书大纲时若项目绑定参考书，则改走异步 reference-outline 任务并轮询结果，再进入原有确认大纲/分卷流程。验证：`npx eslint src/components/project/NewProjectModal.tsx src/components/workspace/DesktopWorkspace.tsx` 0 error（保留既有 warning），`npm run build` 通过。
