...
const OPERATIONS: { key: Operation; labelKey: MessageKey }[] = [
  { key: 'condense', labelKey: 'editor.rewrite.condense' },
  { key: 'expand', labelKey: 'editor.rewrite.expand' },
  { key: 'restructure', labelKey: 'editor.rewrite.restructure' },
  { key: 'continue', labelKey: 'editor.rewrite.continue' },
  { key: 'custom', labelKey: 'editor.rewrite.custom' },
]
      ...
      {showCustomInput && !showPreview && (
        <div className="mt-1 bg-white rounded-lg shadow-lg border border-gray-200 p-2.5 w-72">
          <textarea
            ...
            placeholder={t('editor.rewrite.customPlaceholder')}
      ...
      {showPreview && (
        <div className="bg-white rounded-lg shadow-lg border border-gray-200 p-3 w-96 max-h-80 overflow-y-auto">
          <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">
            {t('editor.rewrite.preview')}
          </h4>

          {/* Original text with strikethrough */}
          <div className="mb-2">
            <span className="text-[10px] font-medium text-red-500 uppercase">{t('editor.rewrite.original')}</span>
          ...
          <div className="mb-3">
            <span className="text-[10px] font-medium text-green-600 uppercase">
              {isLoading ? t('editor.rewrite.generating') : t('editor.rewrite.new')}
            </span>
            <div className="mt-0.5 px-2 py-1.5 bg-green-50 border border-green-100 rounded text-xs text-gray-800 leading-relaxed min-h-[2rem]">
              {error ? (
                ...
                <span className="text-red-500">{t('editor.rewrite.failed')}: {error}</span>
              ) : (
                rewrittenText || (
                  <span className="text-gray-400 animate-pulse">{t('editor.rewrite.generatingText')}</span>
                )
              )}
            </div>
          </div>
