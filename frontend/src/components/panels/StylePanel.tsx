...
export function StylePanel({ value, onChange }: StylePanelProps) {
  ...
  return (
    <div className="p-4 space-y-5">
      <div>
        <h3 className="text-sm font-semibold text-gray-900 mb-3">Style Configuration</h3>
      ...
      {mode === 'profile' && (
        <div className="space-y-4">
          ...
          <div>
            ...
            {loading ? (
              <p className="text-xs text-gray-400">Loading profiles...</p>
            ) : error ? (
              <p className="text-xs text-red-500">{error}</p>
            ) : profiles.length === 0 ? (
              <p className="text-xs text-gray-400">No style profiles available.</p>
            ) : (
              <select
      ...
      {mode === 'manual' && (
        <div>
          ...
          <textarea
            ...
            placeholder="Describe the target writing style... (e.g., concise prose with short sentences, heavy dialogue, literary fiction tone)"
