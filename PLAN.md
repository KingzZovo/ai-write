...
~~### 阶段 B：凌祝 bug 单点修~~（已撤销，原文档基于错误前提，详见下方「事故复盘」）
...
- 同一时间，ch10 `outline_json` 已经登记的真实角色「**纪砥（D-2）**」也不在 `characters` 表的 6 条已有档案（沈棠 / 童缄 / 罗弥 / 裴归尘 / 顾玄礼 / 鸦母）里——主库角色档案和正文一样，没有被生成主路径稳定写回。
...
3. **基于真正落库的内容做角色档案对账**。一旦 ch11 内容真的回到 `chapters.content_text`，再用真实正文驱动「纪砥」等真实角色的档案补齐——而且必须由「章末抽取 LLM pass + 章末实体写回管线」这条主线产生，不允许手填。
4. **回归验证**：ch11 重生成后，`length(chapters.content_text) > 0` 且 `characters` 表里出现 ch11 文本里真正出现的、原先缺档的角色（候选：纪砥）。

### 事故复盘：凌祝 bug 是误读

早期窗口把 `backend/tests/test_v174_p02_chapter_summarizer.py` 里一行 `_clean_summary_output()` 的 fixture 字符串 `"纪砥被凌祝拍走。"` 中的 **「凌祝」** 误认成 ch9-ch11 LLM 输出的真实角色名，沿着任务交接传到三件套，差点把虚构名写进 `characters` 主表。**全仓库 `grep -rn '凌祝'` 只命中这条单元测试 + 文档自身引用**，章节正文 / outline / generation_runs 全部没有这个名字。下次任何「角色档案撕裂」类报告，必须先做：(a) `SELECT length(content_text) FROM chapters WHERE id = ...`、(b) `grep` 该名字在 `chapters.content_text` / `outline_json` / `generation_runs.checkpoint_data` 的真实出现位置——再决定改不改 DB。
