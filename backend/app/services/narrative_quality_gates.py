...
- 节奏呼吸变化：全章禁止恒速白描。至少安排一处明显提速（连续短句群推进急事）和一处明显减速（停顿、留白或一句孤立成段的观察），疏密交替；高信息段落之后要给读者一口气。
- 章末钩子压轴：本章最强的悬念物（未解的异常、新出现的威胁、被发现的痕迹）必须放在全章最后一拍，不得埋在收尾事务性段落（搬运、锁门、盘点）中间；结尾禁止落在人物的日常工作计划上。
- 模糊套话禁用：禁止用“说不清的”“某种”“莫名的”“难以言喻的”“复杂的情绪”来敷衍心理描写；情绪首选落到一个准确的念头或对话内容的变化上；身体反应只是备选，且受“微动作预算”限额约束，不得成为默认出口。
- 微动作预算：每个场景的“小动作拍”（推眼镜/摸物件/关节作响/捏纸角这类）总量受限——每千字不超过 3 处；同一个具体动作（如推眼镜）全章最多出现 1 次；情绪表达优先用“一个准确的念头”或“对话内容的变化”，身体反应是备选而非默认。
- 自然白话优先：禁止刻意把双音节词砍成单字硬凑极简（“稳定”不写成“稳在”、“看清”不写成“读”），除非是人物口语；叙述以自然现代白话为基调，长短句交替，不追求文言式紧缩的酷感。
- 奇喻限额：新奇比喻每章不超过 2 处，其余位置用平实陈述；禁止英语骨架直译式表达（“介于两者之间”“某种意义上”“从某种程度”“事实上”开头）。
- 首现具象化：任何虚构器物/机构/制度第一次出现时，必须当场给一句让读者能在脑中成像的具体描写（形状/大小/材质/操作方式任选其二），不得只给代称加评价（“老壳子”“那东西”）。
- 设定引用自洽：正文引用世界观机制（配额/管制/交易规则）时，交代必须足以让读者理解其运作方式，或干脆不展开；不得留下“读者必须自行脑补机制”的半截设定。
- era_register_consistency（时代语域一致性）：计时/度量/器物/称谓词必须属于本书时代设定。风格参考书只借“手法”（节奏、意象密度、对白张力），禁止把参考书的时代词汇搬进正文：现代/近未来/科幻不写两炷香、一盏茶、时辰、三更天、几丈几里地；古风/仙侠/历史不写分钟、小时、公里、百分比、手机。
- scene_seam_dedup（场景接缝去重）：相邻场景/段落禁止复述同一拍。上一段已写“声音断了、只剩电流声”，下一段不得换个说法再叙述一遍；接缝处只写新增变化、新反应或新信息，复述句必须合并或删除。
"""
