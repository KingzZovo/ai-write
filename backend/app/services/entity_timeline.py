...
输出纯 JSON 格式：
{
  ...
  "state_changes": [
    {"name": "已有角色名", "changes": {"状态": "新状态", "情绪": "新情绪", "位置": "新地点", "持物": ["新增/失去的道具"], "知识边界": ["本章新知"], "未决项": ["新增/消解的承诺"], "生理": "..."}}
  ],
  "new_relationships": [
...
规则：
1. 没有变化的字段一律省略（不要写空串/空数组占位）；整个角色无变化就别出现。
2. "位置/持物/知识边界/称谓/未决项/生理" 是新增的状态维度，只在本章「确实发生变化」时填写。
3. "知识边界" 记录本章角色「新知晓的关键事实」（用于跨章一致性校验，防穿帮）。
4. "未决项" 记录本章埋下但未消解的承诺/选择/伏笔，便于后续兑现。
5. 没有相应变化的顶层字段返回空数组。
