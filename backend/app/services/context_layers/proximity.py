...
async def build_proximity_layer(
    ...
    try:
        # --- Core chapter data (B-class: critical) ---
        try:
            ...
            logger.error(
                "Core chapter query failed [project=%s]: %s", pid, e
            )
            return
        ...
        try:
            ...
            logger.warning("Failed to load book outline [project=%s]: %s", pid, e)
        ...
        try:
            ...
            logger.warning("Failed to load volume outline [project=%s]: %s", pid, e)
        ...
        if chapter_idx <= 3:
            try:
                ...
                logger.warning(
                    "Failed to load volume summaries [project=%s]: %s", pid, e
                )

    except Exception as e:
        logger.warning("Failed to build proximity layer [project=%s]: %s", pid, e)
